...
namespace MC_BSR_S2_Calculator.Utility.TextBoxes {
    ...
    public abstract class NumberTextBox : TypedTextBox<double> {
        ...
        public double MaxInput {
            get => (double)GetValue(MaxInputProperty);
            set => SetValue(MaxInputProperty, value);
        }

        public static readonly DependencyProperty MaxInputProperty = DependencyProperty.Register(
            nameof(MaxInput),
            typeof(double),
            typeof(NumberTextBox),
        ...
        public double MinInput {
            get => (double)GetValue(MinInputProperty);
            set => SetValue(MinInputProperty, value);
        }

        public static readonly DependencyProperty MinInputProperty = DependencyProperty.Register(
            nameof(MinInput),
            typeof(double),
            typeof(NumberTextBox),
        ...
        public override void Validate(object? sender, EventArgs args) {
            ...
            try {
                ...
                // check if the final results are above max or below min
                finalResult = Math.Max(finalResult, MinInput);
                finalResult = Math.Min(finalResult, MaxInput);

                // set text and cleaned text
                textBox.Text = finalResult.ToString();
            } catch (OverflowException) {
                // the number got too big at somepoint
                RevertText(textBox);
                return;
            }
